...
class PlatformerControllerJumping {
	...
	//The sound to fire when jumping
	var jumpSoundEffect:AudioClip;
}
...
function ApplyJumping () {
	...
	if (controller.isGrounded) {
		...
		if (jump.enabled && Time.time < jump.lastButtonTime + jump.timeout) {
			...
			if(jump.jumpSoundEffect){
				//camera.SendMessage("PlaySoundEffect",jump.jumpSoundEffect);
				//jump.jumpSoundEffect.Play();
				AudioSource.PlayClipAtPoint(jump.jumpSoundEffect, Vector3 (26, 24, -177));
			}
